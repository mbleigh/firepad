...
export interface ITextOperation {
  apply();
  invert();
  compose(other: ITextOperation);
  // convenience method to write transform(a, b) as a.transform(b)
  transform(other: ITextOperation);
}
...
export class TextOperation {
  ...
  apply(str: string, oldAttributes, newAttributes) {
    var operation = this;
  ...
  compose(operation2: TextOperation) {
    ...
    function composeAttributes(first, second, firstOpIsInsert = false) {
      var merged = {},
        attr;
      for (attr in first) {
